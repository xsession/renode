...
//
// PIC16 Timer1 — 16-bit timer/counter.
// Registers: TMR1L (0x00), TMR1H (0x01), T1CON (0x02).
//
using System;
...
{
    public class PIC16_Timer1 : IBytePeripheral, IKnownSize
    {
        public PIC16_Timer1(IMachine machine, long frequency = 4_000_000)
        {
            IRQ = new GPIO();
        ...
        public byte ReadByte(long offset)
        {
            switch(offset)
            {
                case 0x00: return (byte)(timer.Value & 0xFF);
                case 0x01: return (byte)((timer.Value >> 8) & 0xFF);
                case 0x02: return t1con;
            }
            this.Log(LogLevel.Warning, "PIC16_Timer1: Read 0x{0:X}", offset);
            return 0;
        }
